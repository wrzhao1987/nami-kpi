...
{% block content %}
    ...
    <fieldset>
    {% if notice['code'] != 403 %}
        {{ form('/exchange/editType?type_id=' ~ type_id, 'method':'post', 'id':'exchangeform', 'name':'exchangeform') }}
        <table style="margin: 0 auto; width: 100%;" border="1">
            <tr>
                <td>
                    <label for="title">礼包名称:</label>
                    {{ text_field("name", "size":20, "value":curr_type.name) }}
                </td>
                <td>
                    <label>适用渠道:</label>
                    <select id="channel_id" name="channel_id">
                        <option value="1" {% if curr_type.channel_id == 1 %}selected="selected"{% endif %}>安卓区</option>
                        <option value="2" {% if curr_type.channel_id == 2 %}selected="selected"{% endif %}>苹果区</option>
                        <option value="3" {% if curr_type.channel_id == 3 %}selected="selected"{% endif %}>越狱区</option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>
                    ...
                    {{ text_field("start_date", "readonly": "readonly", "value": curr_type.start_date) }}
                </td>
                <td>
                    <label>结束日期:</label>
                    {{ text_field("end_date", "readonly": "readonly", "value": curr_type.end_date) }}
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    ...
                    <select id="reuse" name="reuse">
                        <option value="1"{% if curr_type.reuse == 1 %}selected="selected"{% endif %}>可重复多次使用</option>
                        <option value="2"{% if curr_type.reuse == 1 %}selected="selected"{% endif %}>使用后失效</option>
                    </select>
                </td>
            </tr>
            <tr>
        ...
        {{ submit_button("修改", "id": "generate", "name": "generate", "onclick": "return submit_check();") }}
        {{ endform() }}
        </fieldset>
    {% endif %}
{% endblock %}
